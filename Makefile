...
ifeq ($(CHAPTER), 3)
	make -C user bin CHAPTER=3_0
	make -C ../os clean
	make -C ../os run | tee stdout-ch3_0
	python3 check/ch3_0.py < stdout-ch3_0

	make -C user bin CHAPTER=3_1
	make -C ../os clean
	make -C ../os run | tee stdout-ch3_1
	python3 check/ch3_1.py < stdout-ch3_1

	make -C user bin CHAPTER=3_2
	make -C ../os clean
	make -C ../os run | tee stdout-ch3_2
	python3 check/ch3_2.py < stdout-ch3_2
else ifeq ($(CHAPTER), 7)
	make -C user all CHAPTER=7
	make -C ../os clean
	make -C ../os run | tee stdout-ch7
	python3 check/ch7.py < stdout-ch7
else 
	make -C user bin CHAPTER=$(CHAPTER)
	make -C ../os clean
